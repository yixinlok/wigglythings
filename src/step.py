...
def wp_update_all_instances(bm, bi, ix, frame_i):
    ...
    def wp_get_rot_transpose(
            ...
            ix_rot_T: wp.array3d(dtype=wp.float32)
        ):
        i = wp.tid()
        ...
        ix_rot_T[i][0][0] = rot_matrix[0,0]
        ix_rot_T[i][0][1] = rot_matrix[1,0]
        ix_rot_T[i][0][2] = rot_matrix[2,0]
        ix_rot_T[i][1][0] = rot_matrix[0,1]
        ix_rot_T[i][1][1] = rot_matrix[1,1]
        ix_rot_T[i][1][2] = rot_matrix[2,1]
        ix_rot_T[i][2][0] = rot_matrix[0,2]
        ix_rot_T[i][2][1] = rot_matrix[1,2]
        ix_rot_T[i][2][2] = rot_matrix[2,2]  
...
def wp_dyrt(bm, bi, ix, frame_i):
    ...
    wp.launch(wp_get_third_terms,
            dim=ix.num_instances,
            inputs=[ix.accelerations, bi.mtm],
            outputs=[ix.third_terms],
            device=DEVICE)
    ...
    wp.launch(get_q_new, 
                ...
                inputs=[c1,c2,c3, ix.q_cur, ix.q_prev, ix.third_terms], 
                outputs=[ix.q_new], 
                device=DEVICE)

    ix.instances_update_q(ix.q_new)
